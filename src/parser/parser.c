...
int calculate_instruction_size(Token* tokens, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (tokens[i].kind == TOK_LITERAL) {
            ...
            case LIT_LONG_LONG:
                instruction_size += sizeof(tokens[i].value.operand.value.literal.value.longValue);
            case LIT_CHAR:
                instruction_size += sizeof(tokens[i].value.operand.value.literal.value.charValue);
            case LIT_STR:
                instruction_size += sizeof(tokens[i].value.operand.value.literal.value.stringValue);
            }
        }
    }
